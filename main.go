...
func main() {
        ...
        if *promptFlag != "" {
                if _, err := runPrompt(ctx, state.Genkit, bashTool, systemOutput, chat, *promptFlag, thinkingConfig); err != nil {
        ...
        for {
                ...
                updatedChat, err := runPrompt(ctx, state.Genkit, bashTool, systemOutput, chat, prompt, thinkingConfig)
                if err != nil {
